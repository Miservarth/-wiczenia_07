...
```{r}
drzewa <- read_delim("https://raw.githubusercontent.com/pielechr/Wprowadzenie_do_R/refs/heads/main/drzewa.csv")
species_list <- read_delim("https://raw.githubusercontent.com/pielechr/Wprowadzenie_do_R/refs/heads/main/species_list.csv")
traits <-read_delim("http://raw.githubusercontent.com/pielechr/Wprowadzenie_do_R/refs/heads/main/traits.csv")
```
...
drzewa
species_list
traits #w ekologicznym slangu znaczy cechy danego osobnika lub populacji
...
```{r}
species_list %>%                               # lewa tabela
  right_join(drzewa,                            # wskazujemy prawą tabelę
...
```{r}
species_list %>%                               # lewa tabela
  inner_join(drzewa,                            # wskazujemy prawą tabelę
            join_by(species == nazwa_naukowa))
...
```{r}
species_list %>%                               # lewa tabela
  full_join(drzewa,                            # wskazujemy prawą tabelę
            join_by(species == nazwa_naukowa))
...
```{r}
species_list %>%                               # lewa tabela
  left_join(traits,                            # wskazujemy prawą tabelę
            by = "species")
...
```{r}
ssaki <- read_delim("http://raw.githubusercontent.com/pielechr/Wprowadzenie_do_R/refs/heads/main/ssaki.csv")
drapiezniki1 <- drapiezniki %>% 
  filter(year > 2020)
wyniki <- drapiezniki1 %>% 
  left_join(ssaki,
            join_by(species == latin)) %>% 
  select(species, polish_name, everything()) %>% 
...
wyniki_wide<- wyniki %>% 
  pivot_wider(
    ...
    values_from = n_obs       # liczba obserwacji to będą wartości
  )
...
iris %>% 
  ggplot(aes(x = Species, y = Petal.Length, fill = Species)) + 
  geom_boxplot(notch = T, outlier.size = 2, outlier.shape = 21)
...
Zmodyfikujemy teraz wyswietlane etykiety za pomocą funkcji `labs()`

```{r}
plot +
  scale_fill_manual(values = c("red", "green", "yellow")) +
  labs(x = "gatunek", y = "długość płatków",
       title = "Iris")
```
